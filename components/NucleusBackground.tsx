...
export interface NucleusBackgroundHandle {
  /** Cycle the nucleus's visual state (rings -> text -> scatter -> rings). Pass the
   *  screen coordinates of the tap/click so the effect only fires near the nucleus. */
  interact: (clientX: number, clientY: number) => void
}
...
const NucleusBackground = forwardRef<NucleusBackgroundHandle>(function NucleusBackground(_props, ref) {
  const canvasRef    = useRef<HTMLCanvasElement>(null)
  const stateRef     = useRef<NucleusState>(0)
  const cursorRef    = useRef<HTMLDivElement>(null)
  const interactRef  = useRef<((clientX: number, clientY: number) => void) | null>(null)

  useImperativeHandle(ref, () => ({
    interact: (clientX: number, clientY: number) => interactRef.current?.(clientX, clientY),
  }), [])
  ...
  useEffect(() => {
    ...
    // Interact — cycle state. Called only from the dedicated tap layer in Desktop.tsx,
    // never from a blind window-level click listener, so it can't compete with real buttons.
    const onInteract = (clientX: number, clientY: number) => {
      const dx   = clientX - cx()
      const dy   = clientY - cy()
    ...
    const onResize = () => { resize(); buildTextPositions() }
...
export default NucleusBackground