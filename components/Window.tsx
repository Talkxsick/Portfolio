...
export default function Window({
  ...
  const onTitleBarMouseDown = useCallback((e: React.MouseEvent) => {
    if (isMobile) return // mobile uses swipe-down-to-dismiss instead of free dragging
  ...
  }, [id, setPosition, focusWindow, isMobile])
  ...
  return (
    <AnimatePresence>
      <motion.div
        ref={windowRef}
        key={id}
        initial={isMobile ? { opacity: 1, y: '100%' } : { opacity: 0, scale: 0.92, y: 12 }}
        animate={isMobile ? { opacity: 1, y: 0 }      : { opacity: 1, scale: 1,    y: 0  }}
        exit={   isMobile ? { opacity: 1, y: '100%' } : { opacity: 0, scale: 0.92, y: 12  }}
        transition={isMobile ? { type: 'spring', damping: 32, stiffness: 340 } : { duration: 0.12 }}
        onMouseDown={() => focusWindow(id)}
        data-window="true"
        drag={isMobile ? 'y' : false}
        dragElastic={{ top: 0, bottom: 0.35 }}
        dragConstraints={{ top: 0, bottom: 0 }}
        onDragEnd={isMobile ? onSheetDragEnd : undefined}
        style={isMobile ? {
          position: 'fixed',
      ...
      >
        ...
        <div style={{
          ...
          borderLeft: isMobile ? 'none' : `1px solid ${ORANGE_DIM}`,
          borderRight: isMobile ? 'none' : `1px solid ${ORANGE_DIM}`,
          borderBottom: isMobile ? 'none' : `1px solid ${ORANGE_DIM}`,
        ...
        <div style={{
          ...
          {isMobile && (
            <div style={{
              ...
              padding: '8px 0 2px', position: 'relative', zIndex: 11,
            }}>
              <div style={{ width: 36, height: 4, borderRadius: 2, background: 'rgba(255,107,0,0.3)' }} />
            </div>
          )}
          ...
          {/* Bottom status bar */}
          <div style={{
            height: isMobile ? undefined : 22,
            minHeight: 22,
            flexShrink: 0,
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            padding: isMobile ? '5px 14px calc(5px + env(safe-area-inset-bottom, 0px))' : '0 14px',
